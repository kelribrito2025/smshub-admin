...
## 🐛 Bug: Notificação de Saldo Adicionado Não Aparece

- [x] Investigar por que usuário não recebe notificação quando admin adiciona saldo
- [x] Verificar se backend está enviando notificação SSE corretamente
- [x] Verificar se frontend está recebendo e processando notificação
- [x] Verificar logs do servidor durante adição de saldo
- [x] Testar com admin no Chrome e usuário no Safari (navegadores diferentes)
- [x] Corrigir bug identificado (flag playSound não era respeitado no toast)
- [x] Criar testes unitários (4 testes passaram)
- [x] Testar fluxo completo manualmente: admin adiciona saldo → usuário recebe notificação
- [ ] PROBLEMA IDENTIFICADO: SSE não está entregando notificações em tempo real
- [ ] Diagnosticar por que conexão SSE não funciona (saldo só atualiza após F5)
- [ ] Verificar logs do servidor durante adição de saldo
- [ ] Verificar console do navegador (Safari) para erros de SSE
- [ ] Corrigir conexão SSE
- [ ] Testar novamente

## 🔕 Remover Notificação de Saldo Adicionado

- [x] Remover envio de notificação SSE no backend (customers.addBalance)
- [x] Remover lógica de toast verde + som no frontend (useNotifications)
- [x] Áudio money-received.mp3 mantido (pode ser usado para outras notificações)
- [x] Atualizar testes unitários (balance-notification.test.ts) - 4/4 passaram
- [x] Testar: admin adiciona saldo → saldo atualiza silenciosamente (sem toast, sem som) ✅
- [x] Limpar código de debug (console.log adicionados)
