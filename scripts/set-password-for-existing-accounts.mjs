 ...
 * Uso: node scripts/set-password-for-existing-accounts.mjs <email> <password>
 */
...
if (!email || !password) {
  console.error('❌ Uso: node scripts/set-password-for-existing-accounts.mjs <email> <password>');
  console.error('   Exemplo: node scripts/set-password-for-existing-accounts.mjs user@example.com senha123');
  process.exit(1);
}
