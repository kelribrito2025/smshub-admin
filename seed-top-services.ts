  ...
  for (let i = 0; i < selectedServices.length; i++) {
    ...
    for (let j = 0; j < activationCount; j++) {
      await db.insert(schema.activations).values({
        smshubActivationId: `sim-${Date.now()}-${Math.random().toString(36).substring(7)}`,
        apiId: 1,
        userId: null, // Simulação sem usuário específico
        countryId: 1, // Brasil
        serviceId: service.id,
        phoneNumber: `+5511${Math.floor(900000000 + Math.random() * 100000000)}`,
        status: 'completed',
        smshubStatus: 'STATUS_OK',
        smsCode: '123456',
        smshubCost: 100,
        sellingPrice: 200,
        profit: 100,
